...
| B    | ✅ **Aplicada 2026-05-19** — 19 tablas DGII/POS de `0003_dgii_pos.sql` + función `reserve_ecf_sequence_number` + seeds de `0004` (18 permisos) y `0005` (7 roles + 59 role_permissions) aplicados manualmente desde Dashboard SQL Editor con validador in-transaction. `DATA_SOURCE` sigue en `mock`. | Sólo SQL                                   | Sí (aplicado) | No             | — (paso 1 cerrado)                |
| C    | Ampliar `env.ts` con tres ambientes + `dgii_settings.base_url_*`; persistir `/dgii/configuracion`. Pendiente: regenerar `database.types.ts` con `supabase gen types --project-id <ref>` cuando se llenen credenciales reales en `.env.local`. | env + page + service                        | Sí      | No             | Fase B (ya cerrada)               |
